...
import { glob, readFile, rm, access } from "node:fs/promises";
...
const testInputDir = async (inputDirPath: string) => {
  ...
  const testDefaults: DeepPartial<YetiConfig> = {
  ...
  let fileConfig: DeepPartial<YetiConfig> = {};
  const configFilePath = resolve(resolvedInputDir, "yeti.config.ts");
  const configFileExists = await access(configFilePath).then(() => true, () => false);
  if (configFileExists) {
    const configModule = await import(pathToFileURL(configFilePath).href);
    fileConfig = configModule.default ?? {};
  }

  const eleventy = getEleventyInstance(
    resolvedInputDir,
    siteOutputDir,
    mergeConfigs(testDefaults, fileConfig),
  );
...
describe("Yeti Plugin", () => {
  ...
  test("Markdown extension", async () => {
    await testInputDir("../../test_data/markdown");
  });
});
